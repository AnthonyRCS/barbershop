...
export default function CustomerLoginPage() {
  ...
  async function onSubmit(data: FormValues) {
    setServerError("");
    try {
      const res = await fetch("/api/proxy/api/v1/customer-portal/auth/login", {
        method: "POST",
      ...
      if (!res.ok || !payload.token) {
        setServerError(payload.error?.message ?? "Credenciales inválidas");
        return;
      }
      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            ...
            <p className="mt-0.5 text-sm text-zinc-500">Consulta tus citas y tu historial</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/80 shadow-2xl shadow-black/50 backdrop-blur-sm">
          <div className="border-b border-zinc-800 px-6 py-4">
            ...
            <p className="mt-0.5 text-xs text-zinc-500">Accede a tu portal de cliente</p>
          </div>
